...
class Planner:
    def __init__(self, map_shape: tuple[int, int], view_radius: int):
        ...
        self.rrt_neighbor_radius = 2 * self.rrt_step_size
    ...
    def plan_next_step(self, current_pos: tuple[int, int], all_possible_targets: list[tuple[int, int]]) -> tuple[
        int, int]:
        ...
        if self.frontier_points:
            # 使用RRT*找到最近的边界点
            closest_frontier = self.find_closest_frontier_rrt_star(current_pos)
            if closest_frontier:
                # 使用A*规划到边界点的路径
    ...
    def find_closest_frontier_rrt_star(self, start_pos: tuple[int, int]) -> tuple[int, int]:
        """使用RRT*算法找到最近的边界点"""
        ...
        # 创建RRT*树
        tree = {}  # 节点: (父节点, 路径代价)
        tree[start_pos] = (None, 0)  # 根节点无父节点，代价为0
        nodes = [start_pos]
        ...
        for _ in range(self.rrt_max_nodes):
            ...
            # 检查路径是否有效（无已知障碍）
            if not self.is_valid_path(nearest_node, new_node):
                continue

            # 步骤1: 寻找附近节点作为潜在父节点
            neighbor_nodes = [
                node for node in nodes
                if self.euclidean_distance(node, new_node) <= self.rrt_neighbor_radius
                   and self.is_valid_path(node, new_node)
            ]

            # 初始化新节点的父节点和代价
            min_cost = tree[nearest_node][1] + self.euclidean_distance(nearest_node, new_node)
            best_parent = nearest_node

            # 在邻居中寻找最优父节点
            for neighbor in neighbor_nodes:
                cost = tree[neighbor][1] + self.euclidean_distance(neighbor, new_node)
                if cost < min_cost:
                    min_cost = cost
                    best_parent = neighbor

            # 添加新节点到树
            tree[new_node] = (best_parent, min_cost)
            nodes.append(new_node)

            # 步骤2: 重新连接（rewire）邻居节点
            for neighbor in neighbor_nodes:
                cost = min_cost + self.euclidean_distance(new_node, neighbor)
                if cost < tree[neighbor][1]:
                    tree[neighbor] = (new_node, cost)
